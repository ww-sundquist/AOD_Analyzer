...
##	2. For samplesize number of instances, pick a number on [startX,endX] with probability density function given by my piecewise_func()
##	3. For samplesize number of instances, (A) pick a number on [startX,endX] with pdf given by my piecewise_func(); (B) pick a number on [0,1] with equal probability, take the natural log and multiply by c\tau; and (C) multiply the results of A and B together
...
# Questions:
#	-- Why is the chi2 min ~3x larger than expected?
#	-- Is it the gaussian part of the fit and/or the pointiness that causes the issue?
#	-- Why do I need a fit/pdf at all? Can I just multiply each entry in a beta*gamma bin by an ln-type object and put those in a new hist?